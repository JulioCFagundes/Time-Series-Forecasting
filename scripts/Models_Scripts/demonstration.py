...
'''
dataset_path =  '../../Open_datasets/Crude_Oil.csv'
# save_path= '../../closed_datasets/Closed_results/TESTS_RESULTS.csv'
freq = 'Monthly'
save_path = '../../Crude_Oil_Open_results'
dataset_name = 'crude_oil'
## Calling the dataset
dataset = pd.read_csv(dataset_path)


fillnamethod = 'forward_fill'
dataset.set_index('Date', inplace=True)
dataset.index = pd.to_datetime(dataset.index)
dataset.dropna(inplace=True)
dataset.resample('D').agg('mean')
dataset['value'] = dataset['value'].fillna(method='ffill')
train_window_size = 24
test_window_size = 3
value_description = 'default'
...
##################################################################################
'''
This part is for ARGUS BASE
'''
dataset_path =  '../../Open_datasets/Crude_Oil.csv'
save_path= '../../closed_datasets/Closed_results/TESTS_RESULTS.csv'
freq = 'Monthly'
save_path = '../../closed_datasets'
## Calling the dataset
dataset = pd.read_csv(dataset_path)
...
for product in products:
    dataset[dataset.CODE_NAME == product] = dataset[dataset.CODE_NAME == product].fillna(method='ffill')
...
for product_name in products:
    ...
    time_series = TimeSeries(name=dataset_name, series=dataset['value'], train_window_size=train_window_size, test_window_size=test_window_size, frequency=freq, fillna=fillnamethod, value_description = value_description)
