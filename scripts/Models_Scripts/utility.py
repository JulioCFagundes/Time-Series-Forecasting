...
>>>>>>> fee16cd45c8185069acf949c1dc6c7249a1b5d6a
    ...
    def linear_regression(self):
        ...
        for i in range(len(self.predictions)):
            ...
            print(f'predictwindow:{predict}')
            print(f'trainwindow:{[train[-1]]}')
            ForecastTendencyFittingArray = np.concatenate(([train[-1]],predict))
            print(f'ForecastTendencyFittingArray1: {ForecastTendencyFittingArray}')
            ForecastTendencyFittingArray =  np.array(ForecastTendencyFittingArray).reshape(-1,1)
            print(f'ForecastTendencyFittingArray2: {ForecastTendencyFittingArray}')
            ...
            print(f'TestTendencyFittingArray: {TestTendencyFittingArray}')
...
<<<<<<< HEAD
    ...
    def confidence_interval_1st(self):
        ...
        forecast = self.predictions[:,0]
        ## first_error
        mean = forecast.mean()
        std_deviation = st.stdev(forecast)
        sample_size = len(forecast)
        root_sample_size = sqrt(sample_size)
        upper_confidence_interval_95percent = mean + 12.706*std_deviation/root_sample_size
        lower_confidence_interval_95percent = mean - 12.706*std_deviation/root_sample_size
        upper_confidence_interval_99percent = mean + 63.65*std_deviation/root_sample_size
        lower_confidence_interval_99percent = mean - 63.65*std_deviation/root_sample_size
    ...
    def confidence_interval_2nd(self):
        ...
        sample_size = len(forecast)
        root_sample_size = sqrt(sample_size)
        upper_confidence_interval_95percent = mean + 6.31*std_deviation/root_sample_size
        lower_confidence_interval_95percent = mean - 6.31*std_deviation/root_sample_size
        upper_confidence_interval_99percent = mean + 31.82*std_deviation/root_sample_size
        lower_confidence_interval_99percent = mean - 31.82*std_deviation/root_sample_size

        return upper_confidence_interval_95percent, lower_confidence_interval_95percent, upper_confidence_interval_99percent, lower_confidence_interval_99percent
    
    def confidence_interval_3rd(self):
        ## x = média +- t*(std_deviation)/ root(sample_size), t = 12.706 
        forecast = self.predictions[:,2]
        ## first_error
        mean = forecast.mean()
        std_deviation = st.stdev(forecast)
        sample_size = len(forecast)
